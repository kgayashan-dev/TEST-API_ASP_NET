...
public class EmployeeController : ControllerBase
{
    ...
    /// Requires an active session established via /Auth/Login.
    ...
    public async Task<IActionResult> UpsertEmployee([FromBody] EmployeeUpsertModel model)
    {
        ...
        try
        {
            ...
            // Variable to hold the required function ID for this operation:
            // 2 = EMP_CREATE (Create Employee)
            // 3 = EMP_UPDATE (Update Employee)
            int functionId = isUpdate ? 3 : 2;

            var loginId = HttpContext.Session.GetInt32("LoginID");
            if (!loginId.HasValue)
            {
                return Unauthorized(new { message = "Authentication required. Please log in." });
            }

            // Check that the logged user has the assigned function ID
            var hasPermission = await _dbConnect.CheckUserHasFunctionAsync(loginId.Value, functionId);
            if (!hasPermission)
            {
                var functionCode = isUpdate ? "EMP_UPDATE" : "EMP_CREATE";
                return Unauthorized(new
                {
                    message = $"Unauthorized. You do not have the required permission for FunctionID {functionId} ({functionCode}).",
                    requiredFunctionId = functionId,
                    requiredFunctionCode = functionCode
                });
            }
            ...
            var procedureParameters = new List<SqlParameter>
            {
                ...
                // Optional audit parameters (defaults to authenticated session user if not passed)
                new("@UserId", SqlDbType.NVarChar, 100) { Value = (object?)model.UserId ?? (object?)HttpContext.Session.GetString("UserName") ?? DBNull.Value },
                new("@UserRole", SqlDbType.NVarChar, 50) { Value = (object?)model.UserRole ?? (object?)HttpContext.Session.GetString("UserTitle") ?? DBNull.Value },
