    ...
    public async Task<IActionResult> UpsertUser([FromBody] UserUpsertModel model)
    {
        ...
        try
        {
            ...
            var procedureParameters = new List<SqlParameter>
            {
                ...
                // Context & Audit parameters (defaults to authenticated session user if not passed)
                new("@UserID", SqlDbType.NVarChar, 100)
                {
                    Value = (object?)model.UserId ?? (object?)HttpContext.Session.GetString("UserName") ?? DBNull.Value
                },
                new("@UserRole", SqlDbType.NVarChar, 50)
                {
                    Value = (object?)model.UserRole ?? (object?)HttpContext.Session.GetString("UserTitle") ?? DBNull.Value
                },
